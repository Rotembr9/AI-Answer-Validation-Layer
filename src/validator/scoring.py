...
_NONURGENT_RE = re.compile(r"\bnon\s*urgent\b|\bnonurgent\b", re.IGNORECASE)
_SLA_SCOPE_RE = re.compile(
    r"\b(urgent|severity\s*1|priority\s*1|p1)\b",
    re.IGNORECASE,
)
_DAY_SCALE_RE = re.compile(
    r"\b(?:(?:one|two|three|1|2|3)\s+)?(?:full\s+)?"
    r"(?:(?:calendar|business)\s+)?days?\b",
    re.IGNORECASE,
)
_HOUR_SCALE_RE = re.compile(
...
_SLA_CLAIM_BOUNDARY_RE = re.compile(
    ...
    r",\s*(?=(?:non\s+urgent|urgent|severity\s*1|priority\s*1|p1)\b)|"
    r"\s+\band\s+(?=(?:non\s+urgent|urgent|severity\s*1|priority\s*1|p1)\b)",
    re.IGNORECASE,
)


def _normalize_sla_text(text: str) -> str:
    """Normalize SLA terms whose spelling affects safety checks."""
    normalized = text.lower().replace("-", " ")
    normalized = _NONURGENT_RE.sub("non urgent", normalized)
    return " ".join(normalized.split())


def _sla_claim_spans(text: str) -> list[str]:
    """Split mixed SLA answers so urgent and non-urgent clauses are checked independently."""
    normalized = _normalize_sla_text(text)
    spans = [p.strip(" ,") for p in _SLA_CLAIM_BOUNDARY_RE.split(normalized)]
    return [s for s in spans if s]


def _has_urgent_scope(text: str) -> bool:
    # Remove the non-urgent phrase so its trailing "urgent" token cannot imply urgent scope.
    without_nonurgent = _NONURGENT_RE.sub(" ", text)
    return bool(_SLA_SCOPE_RE.search(without_nonurgent))


def _has_urgent_day_scale_claim(text: str) -> bool:
    for span in _sla_claim_spans(text):
        if not _has_urgent_scope(span):
            continue
        if _SLA_SCOPE_RE.search(span) and _DAY_SCALE_RE.search(span):
            return True
    return False


def _has_nonurgent_hour_scale_claim(text: str) -> bool:
    for span in _sla_claim_spans(text):
        if _NONURGENT_RE.search(span) and _HOUR_SCALE_RE.search(span):
            return True
    return False
...
def _document_has_urgent_hour_sla(doc_norm: str) -> bool:
    return (
        ...
        and ("severity 1" in doc_norm or "urgent" in doc_norm)
    )


def _document_has_nonurgent_day_sla(doc_norm: str) -> bool:
    return "non urgent" in doc_norm and "2 business days" in doc_norm
...
def supported_safety_flags(answer: str, document: str) -> tuple[bool, float]:
    ...
    # Day-scale response window for urgent/Severity-1/P1 vs document's 4 business hours.
    if _document_has_urgent_hour_sla(d) and _has_urgent_day_scale_claim(a):
        extra = max(extra, 0.92)
        forbid = True

    # Non-urgent SLA must not borrow the urgent 4-business-hour window.
    if _document_has_nonurgent_day_sla(d) and _has_nonurgent_hour_scale_claim(a):
...
def contradiction_signals(
    ...
    a_norm = _normalize_sla_text(answer)
    d_norm = _normalize_sla_text(document)
    penalty = 0.0
    ...
    # SLA mix-ups are checked by claim span so a correct non-urgent clause does not
    # hide a wrong urgent clause, or vice versa.
    if _document_has_urgent_hour_sla(d_norm) and _has_urgent_day_scale_claim(answer):
        penalty = max(penalty, 0.88)
    if (
        _document_has_nonurgent_day_sla(d_norm)
        and _has_nonurgent_hour_scale_claim(answer)

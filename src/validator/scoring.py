...
def _claim_spans(text: str) -> list[str]:
    """Split enough to keep mixed urgent/non-urgent policy claims independent."""
...
def _business_day_counts(text: str) -> set[int]:
    ...
    for m in re.finditer(
        ...
        counts |= tu.extract_numeric_tokens(m.group(1))
    return counts
...
def supported_safety_flags(answer: str, document: str) -> tuple[bool, float]:
    ...
    # section number (e.g. Severity 1) makes the broad numeric check look grounded.
    ...
    # Remote day caps are small numbers; compare the claimed cap to the remote policy
    # line instead of accepting an unrelated "1" or section number elsewhere.

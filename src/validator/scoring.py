...
def _normalize_policy_terms(text: str) -> str:
    """Normalize policy terms that safety rules need to match consistently."""
...
def _sla_claim_spans(text: str) -> list[str]:
    spans = [
        _normalize_policy_terms(part)
        for part in re.split(r"[;\n.]+|\bbut\b|\bwhile\b", text)
        if part.strip()
    ]
    return spans or [_normalize_policy_terms(text)]
...
def _has_urgent_sla_scope(text: str) -> bool:
    ...
    return bool(
        ...
        or re.search(r"\b(severity|priority)\s*1\b", text)
...
def _doc_has_urgent_hour_sla(doc_norm: str) -> bool:
    return (
        ...
        and _has_urgent_sla_scope(doc_norm)
    )


def _doc_has_nonurgent_two_day_sla(doc_norm: str) -> bool:
    return "2 business days" in doc_norm and "non urgent" in doc_norm
...
def supported_safety_flags(answer: str, document: str) -> tuple[bool, float]:
    ...
    if _doc_has_urgent_hour_sla(d):
        for span in _sla_claim_spans(answer):
            if _has_urgent_sla_scope(span) and _has_day_scale_response(span):
                extra = max(extra, 0.92)
                forbid = True

    # Non-urgent tickets must not borrow the urgent business-hour SLA.
    if _doc_has_nonurgent_two_day_sla(d):
        for span in _sla_claim_spans(answer):
            if (
    ...
    # Wrong small caps can slip through number matching because 1 and 2 appear elsewhere.
    if "remote" in a and ("without" in a or "no approval" in a or "no sign off" in a):
        if "up to 3" in d or "3 days" in d:
            if re.search(
                r"\b(?:up to|maximum(?: of)?|no more than|limit(?: is)?|allowed)\s+"
                r"(one|two|1|2)\s+(?:remote\s+)?days?\s+per\s+week\b",
                a,
            ):
                extra = max(extra, 0.92)
                forbid = True
    ...
    if denial and any(_has_urgent_sla_scope(span) for span in _sla_claim_spans(answer)):
...
def contradiction_signals(
    ...
    a_norm = _normalize_policy_terms(answer)
    ...
    # Urgent vs non-urgent SLA mix-ups, scoped by answer clause/span.
    for span in _sla_claim_spans(answer):
        if (
            _has_urgent_sla_scope(span)
            and "2 business day" in span
            and "4 business hours" not in span
        ):
            if _doc_has_urgent_hour_sla(d_norm):
                penalty = max(penalty, 0.88)
        # Non-urgent tickets must not use the urgent SLA window (skip hedged omissions).
        if (
            _has_nonurgent_scope(span)
            and "4 business hours" in span
